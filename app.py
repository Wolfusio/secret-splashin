...
def setup_room(room_code):
    ...
    return render_template_string(html_template, step="setup", room_code=room_code, players=players, is_creator=room['creator'] is None)
...
def waiting_room(room_code):
    ...
    return render_template_string(html_template, step="waiting", room_code=room_code, players=players, is_creator=is_creator, master_option=room['master_option'])
...
def start_game(room_code):
    ...
    # sinon distribution automatique
    names = room['players'][:]
    shuffled = names[:]
...
@app.route("/target/<room_code>", methods=["GET"])
def show_target(room_code):
    ...
    if idx < len(names):
        ...
        return render_template_string(html_template, step="target", player=player, target=target, room_code=room_code)
